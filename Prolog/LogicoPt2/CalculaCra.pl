...
%Escreva um programa Prolog que recebe uma lista de notas, onde cada nota é a média de uma disciplina cursada e calcula o CRA com base nisso.
...
calculaCra([], 0).
calculaCra([A|Y], C):- calculaCra(Y, C1), C is C1 + A.


len([], 0).
len([_|Y], C) :- len(Y, C1), C is C1 + 1.
